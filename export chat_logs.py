...
class ChatLogExporter:
    ...
    def list_chatlog_files(self, remove_filepath: bool = True) -> list:
        ...
        if remove_filepath:
            return [self._remove_filepath_chatlogs(file) for file in os.listdir(self.folder)]
    ...
    def _format_data(self,file_name,  data: dict) -> str:
        ...
        body = self._format_chat_list(data['full_chatlog'])
    ...
    def write_save(self, filename, data: str, overwrite: False) -> bool:
        ...
        with open(filename, "w") as f:
                f.write(data)
        return True
    def export(self, chatlog_file: str, save_filename: str = None, overwrite: bool = False) -> bool:
    ...
    def list_save_files(self, remove_filepath: bool = True) -> list:
        ...
        if remove_filepath:
            return [self._remove_filepath_saves(file) for file in os.listdir(self.save_folder)]
...
class ExportChatMenu:
    def __init__(self, chatlog_folder: str, save_folder: str):
        self.chatlog_folder = chatlog_folder
        self.save_folder = save_folder
        self.chatlog_exporter = ChatLogExporter(chatlog_folder, save_folder)
    ...
    def save_menu(self) -> None:
        ...
        msg_list = [
            ...
            "Type q to exit",
            "Type h to show this message again",]
        msg_list.extend(self.chatlog_exporter.list_chatlog_files())
        message = "\n".join(msg_list)
        print(message)
        while True:
            ans = input("> ")
            ...
            if ans_lower in ("q", "quit"):
                ...
                if filename is None:
                    ...
                    return None 
                if self.chatlog_exporter.export(chat_log_file=ans, save_filename=filename, overwrite=overwrite):
                    print("Exported successfully")
                else:
                   raise DevMessedUpError("Error when exporting file. Please report to the developer that something is wrong with the chat log exporter, specifically the export function")
